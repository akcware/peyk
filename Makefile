.PHONY: db migrate run-gateway run-workers run-sessions test gate-0 gate-1 gate-1-llm gate-2 gate-3 gate-4 gate-5 status lint requeue-failed
...
gate-1: test-db
	uv run pytest -q tests/phase1 -m "not llm"

gate-1-llm: test-db
	uv run pytest -q tests/phase1 -m llm -s
